...
async function getNextSample(setState: Dispatch<SetStateAction<FeedbackState>>) {
...
export function FeedbackScreen() {
    const [state, setState] = useState<FeedbackState>({ status: Status.Loading, sample: null });
    useEffect(() => {
    ...
    return (<LoggedScreenWrapper title="My recordings">
        <LoadingWrapper loaded={state.status !== Status.Loading}>
            {state.sample === null && <div>You have labelled all recordings you can. Thank you!</div>}
            {state.sample !== null && (
